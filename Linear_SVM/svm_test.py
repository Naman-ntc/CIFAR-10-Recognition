...
for lr in learning_rates:
	for rs in regularization_strengths:
		for bs in batch_sizes:
			model = SVMclassifier()
			model.add_data(data[:4800],label[:4800],data[48000:],label[48000:],10)
...
plt.close()
print("And Testing Accuracy is %.4f"%(best_svm.give_prediction_and_accuracy(test_data[1:5],test_label[1:5])['accuracy']))